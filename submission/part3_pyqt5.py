...
# === Inject: initialize shared SQLite backend and use DB-backed handlers ===
init_db()  # ensure DB exists

# Override UI handlers to use the shared DB functions
self.refresh_students = lambda q='': self._refresh_students_db(q)
self.refresh_instructors = lambda q='': self._refresh_instructors_db(q)
self.refresh_courses = lambda q='': self._refresh_courses_db(q)

self.add_student = self._add_student_db
self.add_instructor = self._add_instructor_db
self.add_course = self._add_course_db

self.delete_selected = self._delete_selected_db
self.export_csvs = self._export_csvs_db
# === End Inject ===
        self._build()
        self.refresh_all()
...
# === DB-backed overrides (appended) ===
def _refresh_students_db(self, q: str=''):
    rows = read_students(q)
    self._fill_table(self.s_table, rows)

def _refresh_instructors_db(self, q: str=''):
    rows = read_instructors(q)
    self._fill_table(self.i_table, rows)

def _refresh_courses_db(self, q: str=''):
    rows = read_courses(q)
    self._fill_table(self.c_table, rows)

def _add_student_db(self):
    sid = self.s_id.text().strip()
    name = self.s_name.text().strip()
    age = self.s_age.text().strip()
    email = self.s_email.text().strip()

    msg = self._missing_fields_message([('ID', sid), ('Name', name), ('Age', age), ('Email', email)])
    if msg:
        QtWidgets.QMessageBox.critical(self, 'Validation', msg); return
    if not self.is_valid_email(email):
        email_msg = self._email_error_message(email)
        QtWidgets.QMessageBox.critical(self, 'Validation', email_msg); return
    if not self.is_valid_age(age):
        QtWidgets.QMessageBox.critical(self, 'Validation', 'Age must be an integer between 16 and 120.'); return

    create_student(sid, name, age, email)

    # assign selected courses (if any)
    for cid in self._student_course_tokens():
        register_student(sid, cid)

    self.refresh_students(self.s_search.text())
    self.s_id.clear(); self.s_name.clear(); self.s_age.clear(); self.s_email.clear()
    self.s_courses.clear()

def _add_instructor_db(self):
    iid = self.i_id.text().strip()
    name = self.i_name.text().strip()
    age = self.i_age.text().strip()
    email = self.i_email.text().strip()

    msg = self._missing_fields_message([('ID', iid), ('Name', name), ('Age', age), ('Email', email)])
    if msg:
        QtWidgets.QMessageBox.critical(self, 'Validation', msg); return
    if not self.is_valid_email(email):
        QtWidgets.QMessageBox.critical(self, 'Validation', self._email_error_message(email)); return
    if not self.is_valid_age(age):
        QtWidgets.QMessageBox.critical(self, 'Validation', 'Age must be an integer between 16 and 120.'); return

    create_instructor(iid, name, age, email)
    self.refresh_instructors(self.i_search.text())
    self.i_id.clear(); self.i_name.clear(); self.i_age.clear(); self.i_email.clear()

def _add_course_db(self):
    cid = self.c_id.text().strip()
    name = self.c_name.text().strip()
    iid = self.inst_name.currentData()  # combobox stores instructor_id as data
    if not cid or not name:
        QtWidgets.QMessageBox.critical(self, 'Validation', 'Missing data needed: course id, course name')
        return
    create_course(cid, name, iid if iid else None)
    self.refresh_courses(self.c_search.text())
    self.c_id.clear(); self.c_name.clear()

def _delete_selected_db(self, table, kind: str):
    row = table.currentRow()
    if row < 0:
        return
    key = table.item(row, 0).text()
...
def _export_csvs_db(self):
    export_csvs()
    QtWidgets.QMessageBox.information(self, 'Export', 'CSV files exported next to the database.')
